...
class Game {
    ...
    async turn() {
        ...
        try {
            ...
            let moves = this.check_capture_obligation_for_all()
            if (moves.length === 0) {
                moves = this.get_possible_moves_for_all()
                with_capture = false
            }
            this.print_available_moves(moves)
            let coords = this.get_input();

            // check if chosen move is possible
            while (!this.move_validation(coords, moves)) {
                coords = this.get_input();
            }

            // making a move
            this.make_move(coords, with_capture)
